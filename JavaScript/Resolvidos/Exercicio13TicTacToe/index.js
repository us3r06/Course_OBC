...
let globalPlayer = ''
const skins = ['X', 'O']
const jogadorAleatorio = Math.floor(Math.random() * 2)
...
function init() {
  ...
  const jogadorDaVez = 'Jogador da vez: ' + player + ' :' + playerAtual
  ...
  for (pos of positions) {
    if (pos.every((item) => items.includes(item))) {
      ...
      setTimeout(
        ...
        (pos.length - 1) * 200 + 1000,
      ) // Atraso total antes do alert: último índice * 200 + 1000 (1 segundo)
